...
def main(
    ...
    learn_ls = [train(gpu,woof,lr,size,alpha,mom,eps,epochs,bs,mixup,opt,arch,sa,sym,dump,lrfinder,log,sched_type,ann_start,name_num ,i)
                for i in range(run)]
    
    acc_list = []
    
    for learn in learn_ls:
      acc = learn.recorder.metrics[-1][0]
      acc_list.append(acc)
